(function($) {
    ...
    $.Page1 = function(Setting) {
        ...
        pa1 = {
            ...
            ajaxSuccess: function(msg) {
                switch (pa1.config.ajaxCallMode) {
                    case 2:
                        var TotalRow = 0;
                        var value;
                        ...
                        for (var index = 0; index < length; index++) {
                            ...
                            if (index == 0) {
                                ...
                                pa1.Paging(TotalRow, Options.DataParams.range);
                            }
                        };
            ...
            ajaxCall: function(config) {
                $.ajax({
                    type: pa1.config.type,
                    contentType: pa1.config.contentType,
                    cache: pa1.config.cache,
                    async: pa1.config.async,
                    url: pa1.config.url,
                    data: pa1.config.data,
                    dataType: pa1.config.dataType,
                    beforeSend: function() {
                        // $('#divAjaxLoading').show();
                    },
                    complete: function() {
            ...
            GetList: function(click) {
                ...
                pa1.config.ajaxCallMode = 2;
                pa1.config.method = Options.DataUrl;
                pa1.config.url = pa1.config.baseURL + pa1.config.method;
                pa1.config.data = JSON2.stringify(Options.DataParams);
                pa1.ajaxCall(pa1.config);
                switch (click) {
                    case 'First':
            ...
            Paging: function(TotalRow, Range) {
                ...
                var atOnce = 8;
                var limitPerShow = 15;
                pa1.PageList(chooseCase, noOfPage, atOnce, limitPerShow, tempPageNo);
            },
            PageList: function(CurrentCase, noOfPage, atOnce, limitPerShow, CurrentValue) {
                ...
                if (noOfPage < limitPerShow) {
                    if (noOfPage < atOnce) {
                        ...
                        p += '<span id = "previous1"> < </span>';
                        for (var i = 0; i < atOnce; i++) {
                            p += '<span class="Paging' + pagingID + '"> ' + (i + 1) + ' </span>';
                        }
                        p += '<span id = "next1"> > </span>';
                    }
                }
                else {
                    p += '<span id = "previous1"> > </span>';
                ...
                $('#previous1').off().on('click', function() {
                    var tempval;
                    var me;
                    $('.Paging' + pagingID).each(function(index, value) {
                        if ($(this).hasClass('click')) {
                ...
                $('.Paging' + pagingID).off().on('click', function() {
                    if ($(this).attr('class') != "Paging click") {
                        ...
                        pa1.ClassClick(chooseCase, clickValue);
                    }
                });
                pa1.ClassClick(chooseCase, CurrentValue);
            },
    ...
    $.fn.PagingBrowsed = function(Setting) {
        return $.Page1(Setting);
    };
} (jQuery));
