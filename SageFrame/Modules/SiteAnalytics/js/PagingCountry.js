(function($) {
    ...
    $.Page2 = function(Setting) {
        ...
        pa2 = {
            ...
            ajaxSuccess: function(msg) {
                switch (pa2.config.ajaxCallMode) {
                    case 2:
                        var TotalRow = 0;
                        var value;
                        ...
                        for (var index = 0; index < length; index++) {
                            ...
                            if (index == 0) {
                                ...
                                pa2.Paging(TotalRow, Options.DataParams.range);
                            }
                        };
            ...
            ajaxCall: function(config) {
                $.ajax({
                    type: pa2.config.type,
                    contentType: pa2.config.contentType,
                    cache: pa2.config.cache,
                    async: pa2.config.async,
                    url: pa2.config.url,
                    data: pa2.config.data,
                    dataType: pa2.config.dataType,
                    beforeSend: function() {
                        // $('#divAjaxLoading').show();
                    },
                    complete: function() {
            ...
            GetList: function(click) {
                ...
                pa2.config.ajaxCallMode = 2;
                pa2.config.method = Options.DataUrl;
                pa2.config.url = pa2.config.baseURL + pa2.config.method;
                pa2.config.data = JSON2.stringify(Options.DataParams);
                pa2.ajaxCall(pa2.config);
                switch (click) {
                    case 'First':
            ...
            Paging: function(TotalRow, Range) {
                ...
                var atOnce = 8;
                var limitPerShow = 15;
                pa2.PageList(chooseCase, noOfPage, atOnce, limitPerShow, tempPageNo);
            },
            PageList: function(CurrentCase, noOfPage, atOnce, limitPerShow, CurrentValue) {
                ...
                if (noOfPage < limitPerShow) {
                    if (noOfPage < atOnce) {
                        ...
                        p += '<span id = "previous2"> < </span>';
                        for (var i = 0; i < atOnce; i++) {
                            p += '<span class="Paging' + pagingID + '"> ' + (i + 1) + ' </span>';
                        }
                        p += '<span id = "next2"> > </span>';
                    }
                }
                else {
                    p += '<span id = "previous2"> < </span>';
                ...
                $('#previous2').off().on('click', function() {
                    var tempval;
                    var me;
                    $('.Paging' + pagingID).each(function(index, value) {
                        if ($(this).hasClass('click')) {
                ...
                $('.Paging' + pagingID).off().on('click', function() {
                    if ($(this).attr('class') != "Paging click") {
                        ...
                        pa2.ClassClick(chooseCase, clickValue);
                    }
                });
                pa2.ClassClick(chooseCase, CurrentValue);
            },
    ...
    $.fn.PagingCountry = function(Setting) {
        return $.Page2(Setting);
    };
} (jQuery));
