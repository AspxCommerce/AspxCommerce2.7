...
var WishItemAPI = function () {
    var buttonTemplate = '<label class="i-wishlist cssWishListLabel cssClassDarkBtn"><button type="button" id={ID} onclick="WishItemAPI.CheckWishListUniqueness(${Params})" ><span>' + getLocale(AspxWishItems, "WishList +") + '</span></button></label>';

    var itemDeatilButtonTemplate = '<label class="i-wishlist cssWishListLabel cssClassDarkBtn"><button type="button" id={ID} onclick="WishItemAPI.AddWishItemFromDetailPage(${Params})" ><span>' + getLocale(AspxWishItems, "WishList +") + '</span></button></label>';
    ...
    var getCount = function () {
        ...
        $ajaxCall(true, "CountWishItems", param, function (msg) {
            ...
            if (!$("input:hidden[name='itemwishmenu']").attr('done')) {
                //getCount();               
                $('<a href="' + aspxRedirectPath + setting.WishListPage + pageExtension + '" id="lnkMyWishlist"><i class="i-mywishlist"></i>' + getLocale(AspxWishItems, "My Wishlist") + ' <span class="cssClassTotalCount">[' + count + ']</span></a>').insertAfter($("input:hidden[name='itemwishmenu']"));
                ...
                if (!$("input:hidden[name='dashitemwishmenu']").attr('done') == "1") {
                    ...
                    $('<li id="dashMyWishlist">' + getLocale(AspxWishItems, "Your Wishlist Contains:") + '<a href="' + aspxRedirectPath + setting.WishListPage + pageExtension + '"> <span class="cssClassTotalCount">' + count + ' ' + 'items</span></a>').insertAfter($("input:hidden[name='dashitemwishmenu']"));
    ...
    var CheckWishListUniqueness = function (itemID, itemSKU, elem) {
        ...
        if (Boolean.parse($.trim($(elem).offsetParent().find(".cssClassProductsBoxInfo").attr("costvariantItem")))) {
            ...
            $ajaxCall(true, "CheckWishItems", checkdata, function (data) {
                if (data.d) {
                    csscody.alert('<h2>' + getLocale(AspxWishItems, 'Information Alert') + '</h2><p>' + getLocale(AspxWishItems, 'The selected item already exist in your wishlist.') + '</p>');
                } else {
    ...
    var AddWishItemFromDetailPage = function (itemID, itemSKU, elem) {
        ...
        if ($('#divCostVariant').is(':empty')) {
            ...
            if (ItemDetail.info.IsCombinationMatched) {
                ...
                csscody.alert('<h2>' + getLocale(AspxWishItems, 'Information Alert') + '</h2><p>' + getLocale(AspxWishItems, 'Please choose available variants!') + '</p>');
            }
        }
    };
        ...
        if (AspxCommerce.utils.GetUserName().toLowerCase() != 'anonymoususer') {
            if (costVariantValueIDs == 'yes') {
                ...
                $ajaxCall(true, "CheckWishItems", checkdata, function (msg) {
                    if (msg.d) {
                        csscody.alert('<h2>' + getLocale(AspxWishItems, "Information Alert") + '</h2><p>' + getLocale(AspxWishItems, "The selected item already exist in wishlist.") + '</p>');
                    } else {
                        AddToWishListFromJS(itemID, AspxCommerce.utils.GetClientIP(), AspxCommerce.utils.GetAspxClientCoutry(), costVariantValueIDs);
                    }

                }, function (msg) {
                    csscody.error('<h2>' + getLocale(AspxWishItems, "Error Message") + '</h2><p>' + getLocale(AspxWishItems, "Failed to add item in wishlist!") + '</p>');
    ...
    var AddToWishListFromJS = function (itemID, ip, countryName, costVariantValueIDs) {
        ...
        $ajaxCall(true, "SaveWishItems", adddata, function (data) {
            ...
            csscody.info('<h2>' + getLocale(AspxWishItems, "Successful Message") + '</h2><p>' + getLocale(AspxWishItems, "Item has been successfully added to wishlist.") + '</p>');
